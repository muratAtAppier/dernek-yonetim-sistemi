...
type Term = {
  ...
  members?: Array<{
    member: MemberLite
    role: TermMember['role']
    memberType: TermMember['memberType']
    order: number
  }>
}
type Decision = {
...
export function BoardDetailClient({
  ...
  const [newDecision, setNewDecision] = React.useState<{
    ...
    decisionDate: string
    content: string
  }>({ title: '', decisionNo: '', decisionDate: '', content: '' })

  async function loadTerms(selectFirst = false) {
    setLoading(true)
    try {
      const res = await fetch(
        `/api/${org}/boards/terms?boardId=${encodeURIComponent(boardId)}`,
        { cache: 'no-store' }
      )
      ...
      if ((selectFirst || !sel) && items.length)
        sel = items.find((t) => t.isActive)?.id || items[0].id
  ...
  async function loadTermMembers(termId: string) {
    const res = await fetch(
      `/api/${org}/boards/members?termId=${encodeURIComponent(termId)}`,
      { cache: 'no-store' }
    )
    const data = res.ok ? await res.json() : { items: [] }
    ...
    >
    const mapped: TermMember[] = items.map((x, idx) => ({
      memberId: (x as any).memberId || x.member.id,
      role: x.role,
      memberType: x.memberType || 'ASIL',
      order: x.order ?? idx + 1,
      member: x.member,
    }))
  ...
  async function initializeBoard() {
    ...
    // If no terms exist and user can write, create a default active term
    if (terms.length === 0 && canWrite) {
      await createDefaultTerm()
    }

    loadDecisions()
  }

  async function createDefaultTerm() {
    const currentYear = new Date().getFullYear()
    const payload = {
      boardId,
      name: `${currentYear} Dönemi`,
      isActive: true,
      startDate: `${currentYear}-01-01`,
    }

    const res = await fetch(`/api/${org}/boards/terms`, {
      method: 'POST',
  ...
  function changeRole(memberId: string, role: TermMember['role']) {
    setEditingMembers(
      editingMembers.map((e) => (e.memberId === memberId ? { ...e, role } : e))
    )
  }
  function changeMemberType(
    ...
    setEditingMembers(
      editingMembers.map((e) =>
        e.memberId === memberId ? { ...e, memberType } : e
      )
    )
  }
  function changeOrder(memberId: string, order: number) {
    setEditingMembers(
      editingMembers.map((e) => (e.memberId === memberId ? { ...e, order } : e))
    )
  }
  ...
  async function saveMembers() {
    ...
    const items = editingMembers.map(
      ({ memberId, role, memberType, order }) => ({
        memberId,
        role,
        memberType,
        order,
    ...
    const res = await fetch(`/api/${org}/boards/members`, {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ termId: selectedTermId, items, replace: true }),
    })
    if (!res.ok) {
  ...
  async function deleteTerm(termId: string) {
    ...
    const res = await fetch(
      `/api/${org}/boards/terms?id=${encodeURIComponent(termId)}`,
      { method: 'DELETE' }
    )
  ...
  async function loadDecisions() {
    const res = await fetch(
      `/api/${org}/boards/decisions?boardId=${encodeURIComponent(boardId)}`,
      { cache: 'no-store' }
    )
    const data = res.ok ? await res.json() : { items: [] }
  ...
  async function addDecision(e: React.FormEvent) {
    ...
    if (newDecision.decisionDate)
      payload.decisionDate = newDecision.decisionDate
    if (selectedTermId) payload.termId = selectedTermId
    const res = await fetch(`/api/${org}/boards/decisions`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(payload),
    })
  ...
  async function deleteDecision(id: string) {
    ...
    const res = await fetch(
      `/api/${org}/boards/decisions?id=${encodeURIComponent(id)}`,
      { method: 'DELETE' }
    )
    ...
    <div className="grid gap-6 md:grid-cols-2">
          ...
          <div className="flex items-center gap-2">
            ...
            {selectedTermId && (
              <Button
                size="sm"
                variant="outline"
                onClick={() => setActive(selectedTermId)}
                disabled={!canWrite}
              >
                Aktif Yap
              </Button>
            ...
            {selectedTermId && (
              <Button
                size="sm"
                variant="destructive"
                onClick={() => deleteTerm(selectedTermId)}
                disabled={!canWrite}
              >
                Sil
              </Button>
          ...
          <div className="flex items-center gap-2">
            ...
            <label className="text-xs flex items-center gap-1">
              <input
                ...
                onChange={(e) =>
                  setNewTerm({ ...newTerm, isActive: e.currentTarget.checked })
                }
                disabled={!canWrite}
              />
              Aktif
            </label>
            <Button size="sm" type="submit" disabled={!canWrite}>
              Ekle
            </Button>
          </div>
        </form>
          ...
          {selectedTermId ? (
            <div className="space-y-2">
              ...
              {searchResults.length > 0 && (
                <ul className="max-h-[180px] overflow-auto rounded border">
                  {searchResults.map((m) => (
                    <li
                      key={m.id}
                      className="flex items-center justify-between p-2 border-b last:border-b-0 text-sm"
                    >
                      <div>
              ...
              <div className="rounded border">
                <div className="bg-muted px-2 py-1 text-sm font-medium">
                  Asil Üyeler (
                  {editingMembers.filter((e) => e.memberType === 'ASIL').length}
                  )
                </div>
                <ul className="divide-y">
                  {editingMembers
                    .filter((e) => e.memberType === 'ASIL')
                    .map((e) => (
                      <li
                        key={e.memberId}
                      ...
                      >
                        ...
                        <Button
                          size="sm"
                          variant="destructive"
                          onClick={() => removeMember(e.memberId)}
                          disabled={!canWrite}
                        >
                          Kaldır
                        </Button>
                  ...
                  {editingMembers.filter((e) => e.memberType === 'ASIL')
                    .length === 0 && (
              ...
              <div className="rounded border">
                <div className="bg-muted px-2 py-1 text-sm font-medium">
                  Yedek Üyeler (
                  {
                    editingMembers.filter((e) => e.memberType === 'YEDEK')
                      .length
                  }
                  )
                </div>
              ...
              <div className="flex items-center justify-end gap-2">
                <Button
                  size="sm"
                  variant="outline"
                  onClick={() => loadTermMembers(selectedTermId)}
                >
                  Yenile
                </Button>
                <Button size="sm" onClick={saveMembers} disabled={!canWrite}>
                  Kaydet
                </Button>
              </div>
            </div>
      ...
      <section className="rounded border bg-card p-3">
        ...
        <form
          ...
          className="space-y-2 rounded border p-2 mb-3"
        >
          <Input
          ...
          <div className="flex items-center gap-2">
            <Input
              ...
              onChange={(e) =>
                setNewDecision({ ...newDecision, decisionNo: e.target.value })
              }
              placeholder="Karar No (ops.)"
            ...
            <Input
              ...
              onChange={(e) =>
                setNewDecision({ ...newDecision, decisionDate: e.target.value })
              }
              className="h-8 w-[160px]"
              disabled={!canWrite}
            />
